...
void* threadfunc(void* thread_param)
{
    ...
    thread_func_args->t_id = pthread_self();

    usleep(thread_func_args->wait_to_obtain_ms * 1000);
    
    pthread_mutex_lock(thread_func_args->my_mutex);
    usleep(thread_func_args->wait_to_release_ms * 1000);
    pthread_mutex_unlock(thread_func_args->my_mutex);
    
    thread_func_args->thread_complete_success = true;

    pthread_exit((void *)thread_param);
}
...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
     ...
     */
     ...
     if (my_thread_data == NULL) {
        printf("Thread data creation failure.\n");
        return false;
     }

     my_thread_data->my_mutex = mutex;
     ...
     int rc = pthread_create(thread, NULL, threadfunc, (void *)my_thread_data);
